...
skt_landing.widgets = {
  ...
  widget_accordion: function (ta) {
    ...
    $(widget).each(function () {
      ...
      setOnList.eq(i).find('> .acco-tit button').attr('aria-pressed', 'false');//211029 2022년 앱웹접근성 샘플링 심사 결과 반영 1차_02
      for (var i = 0, leng = setOnList.length; i < leng; ++i) {
        if (setOnList.eq(i).find('> .acco-tit button').length < 1 && _this.find('.acco-cover.disabled').length < 1) {
          ...
          setOnList.eq(i).find('> .acco-tit button').attr('aria-pressed', 'true');//211029 2022년 앱웹접근성 샘플링 심사 결과 반영 1차_02
        }
