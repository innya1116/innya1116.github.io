...
(function($,document){
	...
	//			var maxDate = _thisInput.data('maxDate') || _thisInput.data('selectMaxDate');
		...
		$('.btnCalendarMonth').each(function(idx){
			var _this = $(this);
			var input = _this.siblings('input');
			input.attr('id','month'+idx)
			input.MonthPicker({
				MonthFormat:'yy-mm'
			});
			var button = input.siblings('.month-picker-open-button');
			input.siblings('.month-picker-open-button').css('display','none');
			_this.on('click',function(){
				var id = input.attr('id');
				if(!$('#MonthPicker_'+id+':visible').length){
					button.click();
				}
			});
		});
