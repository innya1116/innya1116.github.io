...
* 셀렉트 박스 제어 함수 ( 기본셀렉트를 지정해서 사용합니다. )
 ...
 슬라이드
 commonJs.slideInit()   							페이지 전체 슬라이드 초기화 					ex) commmonJs.slideInit();
 commonJs.slideMove($(셀렉터 표현식),바꿀 인덱스 번호)	슬라이드 컨텐츠 이동						  ex) commmonJs.slideMove($('.slider:eq(0)'),3);
...
//	var autoLogin = $('#autoLogin');
	...
	function Jessture( obj )
	{
		...
		target.off('touchstart , mousedown').on('touchstart , mousedown' , startH );
...
//						e.preventDefault();
			...
			check();
			body.off('touchmove , mousemove').on('touchmove , mousemove', moveH);
			body.off('touchend , mouseup').on('touchend , mouseup' , endH );
		}
		var ch = false;
		function moveH(e)
		{
...
//					cont.css('margin-left',(marginLeft+cn)+'px');
	...
	commonJs.popShow = function(target)
	{
			...
			if( target.find('.slideArea').length>0  ){
				if( !target.data('slide') ){
					popupSlide = new PopupSlide(target);
				}
				popupSlide.resize();
			}
	}
...
// 		{
// 			var _this = $(this);
// 			_this.find('button.btnClose').off().on('click',function(){
...
//				}()));
		...
		// {
		// 	var _this = $(this);
		// 	_this.find('button.btnClose').on('click',function(){
		// 		fixedBtn.css('display','block')
		// 	});
		// 	calendarCol2.find(calendar).off('click').on('click',function() // 캘린더
		// 	{
		// 		fixedBtn.css('display','none');
		...
		// 					return false;
		// 				}
		// 				var	resultText = dateText + ' ' + timeText;
		// 				_this.siblings('input').val(resultText);
		// 				fixedBtn.css('display','block');
		// 				lycalendar.find('.btnClose').click();
		// 			}
		// 		});
		// 		if(getId === 'lyEndDay')
		// 		{
...
//		});
		doc.find('section.layerPop').not('.dateSelect').each(function(){
			...
			resize.off('resize.pop').on('resize.pop',resizeH)
			function resizeH(){
...
//			_this.find('li.tgl button.tglBtn').on('click',function()/////////////////////////////////////////////////////*2017/08/22 수정*/
	...
	function Slider(){
		...
		this.slideInit = function(){
			slider.each(function()
			{
				...
				var contArea = $('.contArea');
				if( contArea.length>0 && contArea.css('display')=='none' )
				{
					contArea = false;
					contArea.show();
				}
				...
				resize.off('resize.slide').on('resize.slide',resizeH);
				function resizeH()  	/////////////////화면 전환시
				{
					...
					else
					{
						...
						// 2018.04.25 수정 start
						// 한화면에 2개 사용할 경우 화면 깨져서 처리함 LGJ 2018-04-27
						if( !_this.hasClass('noResize') ){
							...
							if(_this.hasClass('couponResize')){
								_thiswW = _thiswW - 30; 
								_thissCon = _thissCon - 30;
								...
								if(_this.hasClass('vehicleSlider')){
									_cont.width(10000);
									_cont.css({width:'10000px',marginLeft:-(sCon.width()*slide.cnt)});
								}
							}
							// _cont.css({'margin-left':-_thiswW*_thisMargin+'px','width':((_thiswW*slideLength)+100)+'px'});
						}
				...
				_this.data('slide',slide);
				resizeH();
				resize.off('resize.jessture').on('resize.jessture',function(){
					slide.resize();
				});
				slide.on(slide.RIGHT,move );
				slide.on(slide.LEFT,move );
				_this.find('button.btnPrev').off('click').on('click',function(){
					slide.next();
				});
				_this.find('button.btnNext').off('click').on('click',function(){
					slide.prev();
				});
				...
				function move(evt){
					...
					//2018.04.28 수정 start
					if( !_this.hasClass('noResize') ){
						_cont.stop().animate({'margin-left':p+'px'},ANIMATE_SPEED-300,ANIMATE_EASING, function (){
							commonJs.event.trigger('changeSlider', [slide.cnt]);
						});
					}else{
						_cont.stop().animate({'margin-left':-(_this.parent().width()*slide.cnt)+'px'},ANIMATE_SPEED-300,ANIMATE_EASING, function (){
							commonJs.event.trigger('changeSlider', [slide.cnt]);
						});
					}
					if( !checkDisplay ){
		...
		this.slideInit();
		this.slideMove = function(target,idx){
			var cont = target.find('.sCon');
			var conW = cont.width();
			var wrap = target.find('.sliderWrap');
			var btnNext = target.find('.btnNext');
			var btnPrev = target.find('.btnPrev');
			target.data('slide').cnt = idx;
			wrap.css('margin-left',-(conW*idx));
			if(idx == 0){
				btnNext.removeClass('none');
				btnPrev.addClass('none');
			}else if(idx == cont.length-1){
	...
	var vehicleSlider = {
		...
		_setElement: function(){
			//초기화
			var $btn = $('.vehicleSlider').find('>button');
			$btn.removeAttr('style');

			var btnPos = parseInt($btn.css('top')) -  parseInt($btn.outerHeight() /2);
			var imgCenter = parseInt($('.vehicleSlider').find('.sCon:visible').find('.couponImg').outerHeight() /2);
			var btnTop = btnPos+imgCenter;

			$btn.css('top', btnTop);
			doc.find('.coupon>.tabType2 li').off('click.coupon').on('click.coupon',function(){
				win.trigger('resize.vehicleSlider');
			});
		},
		_bindEvent: function(){
			$(window).off('resize.vehicleSlider').on('resize.vehicleSlider', $.proxy(this._setElement, this));
		}
	}
	...
	$(function()
	{
		...
		news();
		vehicleSlider.init();
		commonJs.slideInit = slider.slideInit; // 슬라이드 초기화
		commonJs.slideMove = slider.slideMove; // 슬라이드 move
