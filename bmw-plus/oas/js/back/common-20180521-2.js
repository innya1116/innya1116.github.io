...
(function($,window){
	...
	function Jessture( obj )
	{
		...
		if( Math.abs(startX - endX) > 40){}
			...
			{
				cont.stop().animate({'margin-left':marginLeft},ANIMATE_SPEED,ANIMATE_EASING);
			}
			startX = 0;
			body.off( 'touchmove , mousemove' , moveH);
			body.off('touchend , mouseup' , endH );
...
//	}
	...
	{
		/**20180521 팝업 추가**/
		win.on('resize',popResize);
		function popResize(){
			var targetHeight = target.height();
			var windowHeight = $(window).height();
			if( windowHeight < targetHeight + 120 )
			{
				target.css({top : 60, position : 'absolute', marginTop : 0 });
			}
			else
			{
...
//		tglWrap.find('.tglBtn').stop().on('click',function()
		tglWrap.on('click','.tglBtn',function()/////////////////////////////////////////////////////*2017/08/22 수정*/
		{
			...
			if(tgl.hasClass('on'))
			{
				tgl.stop().animate({height:tglBtnHeight},ANIMATE_SPEED-200,ANIMATE_EASING,function()
				{
					tgl.removeClass('on');
				});
			}
			...
			{
				tgl.stop().animate({height:tglConHeight+tglBtnHeight},ANIMATE_SPEED-200,ANIMATE_EASING)
				tgl.addClass('on');
			}
		});
	}
	function common()
	{
		...
		_thisTarget.on('click',function(e)
		{
			...
			hdWrap.stop().animate({height:hdWrapHeight+_thisDepthHeight},ANIMATE_SPEED-200,ANIMATE_EASING);
		});
		var menuHeight = 0;
		...
		linkList.on('click',function(e)
		{
			...
			{
				_thisLinkList.animate({height:linkBtnSize},ANIMATE_SPEED-200,ANIMATE_EASING,function()
				{
			...
			else
			{
				...
				_thisLinkList.animate({height:linkBtnSize+linkListSize},ANIMATE_SPEED-200,ANIMATE_EASING);
			}
		});
		...
		_thisFmsite.on('click',function(e)
		{
			...
			{
				_thisUl.stop().animate({height:0,paddingBottom:0},ANIMATE_SPEED-200,ANIMATE_EASING,function()
				{
			...
			else
			{
				_thisUl.height(0);
				fmsite.addClass('on');
				_thisUl.stop().animate({height:fmsiteSize,paddingBottom:paddingBottom},ANIMATE_SPEED-200,ANIMATE_EASING);
			}
		});
	}
	function slider()
	{
		...
		slider.each(function()
		{
			...
			function resizeH() 	/////////////////화면 전환시
			{
				...
				_cont.css({'margin-left':-wW*_thisMargin+'px','width':((wW*sConLength)+100)+'px'});
	...
	function fixedTab()
	{
		...
		doc.find('.fixedTab').each(function()
		{
			...
			doc.scroll(function()
			{
				...
				{
					_this.css({position:'fixed',top:stickyHeight+'px'})
				}
				else
				{
					_this.css({position:'absolute',top:defaultTop+'px'})
				}
	...
	function Banner()
	{
		...
		var option = {
			...
			TOTAL : el.slideCont.find('li').length/2
		}

		// method
		banner.move = function(idx){
			// console.log( idx );
			el.slideCont.stop().animate({'marginLeft':-option.width*idx},200)
		}
