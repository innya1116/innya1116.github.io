...
(function($, window) {
  ...
  commonJs.scrollLock = function(){
    ...
    $('body').css({position:'fixed',width:'100%',overflow:'hidden',marginTop:-$(window).scrollTop()});
    $(document).find('.stickyWrap').each(function(){
  ...
  function resizeLayerPopup(target,size)
	{
      ...
      // }
			...
			if(size && ((target.attr('id') == 'lycoupon')||(target.attr('id') == 'lyrentpop'))){
				if(size+120 > windowHeight){
          // target.find('.popCont').css({overflowY:'scroll',height:windowHeight-120});
