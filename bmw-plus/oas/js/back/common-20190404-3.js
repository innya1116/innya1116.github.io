...
* 셀렉트 박스 제어 함수 ( 기본셀렉트를 지정해서 사용합니다. )
 ...
 마이페이지 쿠폰
 commonJs.couponInit = coupon.init; // 쿠폰 슬라이드 초기화 ex)commonJs.couponInit();
 commonJs.couponMove = coupon.move; // target 쿠폰 슬라이드 move	ex)commonJs.couponMove(target,idx,speed);
  ...
  function resizeLayerPopup(target,size)
	{
      ...
      var scrollTop = win.scrollTop();
			if( windowHeight < targetHeight + 120 ){
				target.css({top : 60+marginTop+scrollTop, position : 'absolute', marginTop : 0});
			}else{
	...
	function CouponSlide(){
		...
		coupon.init = function(){
			$('.slide.coupon').each(function(){
				var _this = $(this);
				var wrap = _this.children('.slideWrap');
        ...
        var idx = Math.floor(-parseInt(ul.css('margin-left')) / width);
				...
				_this.off('click').on('click','button',function(e){
					e.preventDefault();
					var _this = $(this);
