...
class GnbLayout {
  ...
  constructor(target) {
    ...
    const setProperty = () => {
      ...
      const element = focusElArr.splice(2, 1);
    ...
    const handler = {
      ...
      depthMenu2Active: (event) => {
        ...
        [...el.menuDepthList].forEach((ul) => {
          [...ul.children].forEach((element) => {
            element === activeSubItem ? element.classList.add(className.menu2depth) : element.classList.remove(className.menu2depth);
          });
        });
    ...
    const bind = () => {
      ...
      [...el.menudepthLink].forEach((element) => {
        element.addEventListener('click', handler.depthMenuOpen);
        element.addEventListener('focusin', handler.depthMenuOpen);
      });
      [...el.menuDepthList].forEach((element) => {
        [...element.children].forEach((element) => {
          element.addEventListener('click', handler.depthMenu2Active);
    ...
    const unbind = () => {
      ...
      [...el.menudepthLink].forEach((element) => {
        element.removeEventListener('click', handler.depthMenuOpen);
        element.removeEventListener('focusin', handler.depthMenuOpen);
      });
      [...el.menuDepthList].forEach((element) => {
        [...element.children].forEach((element) => {
          element.removeEventListener('click', handler.depthMenu2Active);
