...
class CustomSwiper {
  ...
  */
  constructor(target) {
    ...
    options.on = {
      paginationUpdate: () =>{
        if (target.querySelectorAll('.swiper-pagination-bullet').length > 1) {
          target.querySelector('.swiper-pagination').style.display = 'block';
        } else {
          target.querySelector('.swiper-pagination').style.display = 'none';
        }
      },

      observerUpdate: () => {
        swiper.slideTo(rIndex, 0);
      },
      ...
      slideChangeTransitionStart: function() {
        ...
        rIndex = swiper.realIndex ? swiper.realIndex : swiper.activeIndex;
      },
 ...
 * @memberof customSwiper
 * @function updateAll
 * @description 대상 swiper 전체 업데이트
 */
  updateAll: id => {
    [... document.querySelectorAll(id)].forEach((el) => {
      const obj = weakMap.get(el);
      // console.log('swiper ', obj);
      if (obj) {
        obj.swiper.update();
      } else {
        weakMap.set(el, new CustomSwiper(el));
