  ...
  constructor(target) {
    ...
    const method = {
      move(target) {
        ...
        let offsetRight = window.innerWidth - li.offsetLeft - li.offsetWidth;

        if (document.dir === 'rtl') {
          if (middle > offsetRight + target.scrollLeft) {
            middle = offsetRight;
          }

          targetEl.querySelector('ul').scrollTo({
            left: middle - offsetRight,
            top: 0,
            behavior: 'smooth'
          });

          // gsap.to(targetEl.querySelector("ul"), {
          //     scrollTo: {x: middle - offsetRight},
          //     duration: 0.4,
    ...
    const bind = () => {
      ...
      el.linkAnchor.forEach((el)=>{
        el.addEventListener('click', e=> handler.clickAnchor(e));
      });
    };
    ...
    const unbind = () => {
      ...
      el.linkAnchor.forEach(el=> {
        el.removeEventListener('click', handler.clickAnchor);
      });
    };
