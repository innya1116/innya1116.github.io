  ...
  constructor(target) {
    ...
    const handler = {
      ...
      toggleClass: (evt) => {
        ...
        //footer가 화면에 보이면 btn-top--fixed 클래스 삭제
        if (!el.footer) {
          return;
        }
        let rectFooter = el.footer.getBoundingClientRect();
        rectFooter.top < window.innerHeight && rectFooter.bottom >= 0 ? el.target.classList.remove('btn-top--fixed') : el.target.classList.add('btn-top--fixed');
      },
      /**
       * @callback scrollTop
       * @memberof TopButton
    ...
    const setProperty = () => {
      el.footer = document.querySelector(selector.footer);
    };
